...
export function VideoPlayer({ slug, fileType, hasAudio, isActive }: Props) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", background: "#111", overflow: "hidden" }}>
      ...
      {/* bottom gradient */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.3) 0%, transparent 35%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none", opacity: showCtrl ? 1 : 0, transition: "opacity 0.2s" }}>
        <div style={{ ...glassDark, borderRadius: "50%", width: 60, height: 60, display: "flex", alignItems: "center", justifyContent: "center" }}>
          {playing ? <PauseIcon size={26} style={{ color: "#fff" }} /> : <PlayIcon size={26} style={{ color: "#fff" }} />}
        </div>
      </div>
      ...
      {/* progress */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "20px 14px 10px", opacity: showCtrl ? 1 : 0, transition: "opacity 0.2s", pointerEvents: showCtrl ? "auto" : "none" }}>
        <div ref={barRef} style={progressTrack}
