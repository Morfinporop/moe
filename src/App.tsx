...
/* ── Ambilight toggle icon ── */
function AmbilightIcon({ size = 20, on }: { size?: number; on: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <rect x="5" y="3" width="14" height="18" rx="2" />
      {on && <>
        <line x1="2" y1="8" x2="2" y2="16" style={{ stroke: "currentColor", opacity: 0.5 }} />
        <line x1="22" y1="8" x2="22" y2="16" style={{ stroke: "currentColor", opacity: 0.5 }} />
        <line x1="3" y1="6" x2="3" y2="18" style={{ stroke: "currentColor", opacity: 0.3 }} />
        <line x1="21" y1="6" x2="21" y2="18" style={{ stroke: "currentColor", opacity: 0.3 }} />
      </>}
    </svg>
  );
}
...
export default function App() {
  ...
  const [ambilight, setAmbilight] = useState(() => {
    const saved = localStorage.getItem("loope_ambilight");
  ...
  /* persist ambilight pref */
  useEffect(() => { localStorage.setItem("loope_ambilight", String(ambilight)); }, [ambilight]);
  ...
  if (!ready) return (
    <div style={{ height: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f2f2f7" }}>
      <div style={{ width: 28, height: 28, border: "2.5px solid #ddd", borderTopColor: "#999", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} />
    </div>
  );
  ...
  return (
    <div style={{ position: "relative", height: "100dvh", overflow: "hidden", background: "#f2f2f7" }}>
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 50, display: "flex", justifyContent: "center", paddingTop: 14, gap: 8, pointerEvents: "none" }}>
        <TopBtn onClick={() => setView("upload")}><PlusIcon size={24} /></TopBtn>
        <TopBtn onClick={() => openProfile(user?.id ?? "")}><UserIcon size={22} /></TopBtn>
        {/* Ambilight toggle — only on desktop */}
        {isWide && (
          <TopBtn onClick={() => setAmbilight(a => !a)}>
            <AmbilightIcon size={20} on={ambilight} />
          </TopBtn>
        )}
      </div>
      ...
      <div ref={feedRef} onScroll={onScroll} style={{ height: "100dvh", overflowY: "scroll", scrollSnapType: "y mandatory", scrollbarWidth: "none" }}>
        {videos.map((v, i) => (
          <div key={v.id} style={{ height: "100dvh", scrollSnapAlign: "start", scrollSnapStop: "always", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", background: "#f2f2f7", overflow: "hidden" }}>

            {/* Ambilight — only on wide + enabled */}
            {isWide && (
              <Ambilight videoEl={idx === i ? activeVideoEl : null} active={idx === i} enabled={ambilight} />
            )}
            ...
            <div style={{ position: "relative", width: vidW, height: "100%", overflow: "hidden", zIndex: 1, borderRadius: isWide ? 6 : 0 }}>
              ...
              {/* bottom info — pinned to very bottom */}
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "40px 14px 16px", pointerEvents: "none", background: "linear-gradient(to top, rgba(0,0,0,0.5) 0%, transparent 100%)" }}>
                ...
                <button onClick={() => openProfile(v.user_id)} style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 6, color: "rgba(255,255,255,0.5)", fontSize: 11, background: "none", border: "none", cursor: "pointer", pointerEvents: "auto" }}>
                  <UserIcon size={11} style={{ color: "rgba(255,255,255,0.5)" }} />
                  {v.user_nickname} · {timeAgo(v.created_at)} · {formatCount(v.views)} views
                </button>
              </div>

              {/* right actions — positioned relative to bottom */}
              <div style={{ position: "absolute", right: 10, bottom: 16, zIndex: 20, display: "flex", flexDirection: "column", gap: 14, alignItems: "center" }}>
                <ActBtn
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 18px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
          {files.length === 0 ? (
            <div onClick={() => fRef.current?.click()} style={{ border: "2px dashed rgba(0,0,0,0.06)", borderRadius: 14, padding: "36px 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: "pointer" }}>
              ...
              <p style={{ color: "#c7c7cc", fontSize: 11 }}>Max 100MB</p>
            </div>
          ) : (
...
function CommentsSheet({ slug, onClose }: { slug: string; onClose: () => void }) {
  ...
  const send = async () => {
    ...
    try { const c = await addComment(slug, text.trim()); setComments(p => [c, ...p]); setText(""); } catch {} setSending(false);
  };
  return (
    <>
      ...
      <div style={{ ...sheet, animation: "slideUp 0.3s cubic-bezier(0.16,1,0.3,1)", zIndex: 100 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 18px", minHeight: 120 }}>
          {loading ? <Spinner /> : comments.length === 0 ? <p style={{ textAlign: "center", color: "#c7c7cc", fontSize: 13, padding: "28px 0" }}>Be the first to comment</p> :
            comments.map(c => (
              <div key={c.id} style={{ display: "flex", gap: 10, marginBottom: 16, animation: "fadeIn 0.2s" }}>
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: `hsl(${(c.user_nickname.charCodeAt(10) || 42) * 47 % 360}, 40%, 72%)`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: "#fff", fontSize: 11, fontWeight: 600 }}>{c.user_nickname.charAt(0).toUpperCase()}</div>
        ...
        <div style={{ padding: "8px 14px 16px", borderTop: "1px solid rgba(0,0,0,0.04)", display: "flex", gap: 6 }}>
          ...
          <button onClick={send} disabled={!text.trim()} style={{ width: 42, height: 42, borderRadius: 12, ...glass, display: "flex", alignItems: "center", justifyContent: "center", opacity: text.trim() ? 1 : 0.35 }}><SendIcon size={16} style={{ color: "#1c1c1e" }} /></button>
        </div>
      </div>
    </>
  );
      ...
      <div style={{ ...modal, maxWidth: 460, animation: "fadeIn 0.2s" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 18px 14px", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 11, color: "#8e8e93", marginTop: 1 }}>{videos.length} videos</p>
          </div>
        </div>
              ...
              {videos.map(v => (
                <div key={v.id} style={{ position: "relative", aspectRatio: "9/16", borderRadius: 8, overflow: "hidden", background: "#111" }}>
                  ...
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "18px 6px 6px", background: "linear-gradient(to top, rgba(0,0,0,0.6), transparent)", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                    ...
                    {isOwn && <button onClick={() => { if (confirm("Delete?")) onDelete(v.slug); }} style={{ width: 22, height: 22, borderRadius: "50%", background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", border: "none" }}><TrashIcon size={10} style={{ color: "#ff6b6b" }} /></button>}
                  </div>
                </div>
              ))}
            </div>
...
function CloseBtn({ onClick }: { onClick: () => void }) {
  return <button onClick={onClick} style={{ width: 30, height: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.04)" }}><XIcon size={14} style={{ color: "#8e8e93" }} /></button>;
}

function Spinner() {
  return <div style={{ display: "flex", justifyContent: "center", padding: 24 }}><div style={{ width: 20, height: 20, border: "2px solid #e5e5ea", borderTopColor: "#8e8e93", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} /></div>;
}
